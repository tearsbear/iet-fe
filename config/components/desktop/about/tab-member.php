<div class="text-center ml-5 mr-5" data-aos="fade-up" data-aos-duration="1300">
    <h3 class="font-inter-bold color1" style="margin-top: 80px">Supporting Partners</h3>
    <p class="text-primary font-inter-light mt-4">
        IET is supported by the Coordinating Ministry for Maritime Affairs and Investment, Ministry of Economic Affairs,
        Ministry of Energy and Mineral Resources, <br /> Ministry of Finance, State-Owned Enterprises Ministry, Ministry
        of
        National Development Planning/National Development Planning Agency, <br /> Ministry of Environment and Forestry,
        PT PLN
        (Persero), and PT Sarana Multi Infrastruktur (Persero).
    </p>
</div>

<div class="row" id="listMember" style="max-width: 80%; margin: 80px auto" data-aos="fade-up" data-aos-duration="1300">

</div>